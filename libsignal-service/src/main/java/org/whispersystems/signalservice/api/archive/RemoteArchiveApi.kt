...
// MOLLY: This is ArchiveApi from upstream
...
class RemoteArchiveApi(
  ...
  companion object {
    @JvmStatic
    fun create(pushServiceSocket: PushServiceSocket, backupServerPublicParams: ByteArray, aci: ACI): RemoteArchiveApi {
      return RemoteArchiveApi(
        pushServiceSocket,
   ...
   */
  override fun listMediaObjects(backupKey: BackupKey, serviceCredential: ArchiveServiceCredential, limit: Int, cursor: String?): NetworkResult<ArchiveGetMediaItemsResponse> {
    return NetworkResult.fromFetch {
      val zkCredential = getZkCredential(backupKey, serviceCredential)
      val presentationData = CredentialPresentationData.from(backupKey, zkCredential, backupServerPublicParams)
